...
import fr.cirad.image.topologicaltracking.RegionAdjacencyGraphPipeline;
import ij.IJ;
import ij.ImagePlus;
...
public class PipelineActionsHandler {
	...
	public static boolean doStepOnImg(int step,int indexImg,PipelineParamHandler pph) {
		...
		if(step==2) {//Registration
			...
			executed=PipelineActionsHandler.registerSerie(outputDataDir,pph);
		}
		if(step==3) {//Compute mask, find leaves falling in the ground and remove them
			...
			executed=PipelineActionsHandler.computeMasksAndRemoveLeaves(outputDataDir,pph);
		}
		if(step==4) {//Compute graph
			t.print("Starting step 4, space/time segmentation -  on img "+pph.imgNames[indexImg]);
			executed=PipelineActionsHandler.spaceTimeMeanShiftSegmentation(outputDataDir,pph);
		}
		if(step==5) {//Compute graph
			t.print("Starting step 5 -  on img "+pph.imgNames[indexImg]);
			executed=PipelineActionsHandler.buildAndProcessGraph(outputDataDir,pph);
		}
	...
	public static boolean stackData(String inputDataDir,String outputDataDir,PipelineParamHandler pph) {
		if(new File(inputDataDir).list().length==0 ) { IJ.showMessage("In PipelineSteps.stackData : No files in InputDataDir "+inputDataDir  );return false;          }
		IJ.run("Image Sequence...", "dir="+inputDataDir+" sort");
		ImagePlus stack=IJ.getImage();
		stack.hide();
		if(stack==null || stack.getStackSize()==0)   { IJ.showMessage("In PipelineSteps.stackData : no stack imported from InputDataDir "+inputDataDir);return false; }		
		IJ.saveAsTiff(stack, new File(outputDataDir,"11_stack.tif").getAbsolutePath());
		return true;
	}	

	public static boolean registerSerie(String outputDataDir,PipelineParamHandler pph) {
	...
	public static boolean computeMasksAndRemoveLeaves(String outputDataDir,PipelineParamHandler pph) {
	...
	public static boolean spaceTimeMeanShiftSegmentation(String outputDataDir,PipelineParamHandler pph) {
	...
	public static boolean buildAndProcessGraph(String outputDataDir,PipelineParamHandler pph) {
		ImagePlus imgDates=IJ.openImage( new File(outputDataDir,"40_date_map.tif").getAbsolutePath());
		RegionAdjacencyGraphPipeline.buildAndProcessGraphStraight(imgDates,outputDataDir,pph);
		return true;
	}

	public static boolean computeRSML(String outputDataDir,PipelineParamHandler pph) {
		...
		ImagePlus regOneSlice=new Duplicator().run(reg,1,1,1,1,1,1);
		...
		rm.writeRSML3D(new File(outputDataDir,"60_graph_no_backtrack.ser").getAbsolutePath(), "",true,false);
		backTrackPrimaries(new File(outputDataDir,"60_graph_no_backtrack.ser").getAbsolutePath(),new File(outputDataDir,"61_graph.ser").getAbsolutePath(),mask,reg,pph.toleranceDistanceForBeuckerSimplification);
		rm=RootModel.RootModelWildReadFromRsml(new File(outputDataDir,"61_graph.ser").getAbsolutePath());
...
//				System.out.println("Testing "+p+ " = "+imgExtractMask.getPixel(p.x, p.y)[0]);
	...
	public static ImagePlus getMenisque(ImagePlus img) {
		...
		int factor=1;//TODO : give a geometrical meaning to 250
