...
//		coord2();
	...
	public static int stackBoites(int[][]series,int numBoite,ImagePlus img) {
		for(int s=0;s<series.length;s++) {
			...
			for(int i=0;i<tab.length;i++) {
				...
				tab[i]=IJ.openImage("/media/rfernandez/DATA_RO_A/Roots_systems/Data_Rootnav/"+numReal+"/image_"+numReal+".tif");
			}
			ImagePlus result=VitimageUtils.slicesToStack(tab);
			IJ.run(result,"Invert","");
			IJ.saveAsTiff(result, "/home/rfernandez/Bureau/A_Test/RSML/0_Stacked/MLRootNav_Boite_"+stringBoite(numBoite)+".tif");
			numBoite++;
		}
		return numBoite;
	}
	...
	public static int verify(int last,int[][]serie) {
		...
		for(int i=0;i<serie.length;i++)for(int j=0;j<serie[i].length;j++) {
			iters[serie[i][j]]++;
		}
	...
	public static void coord3() {
		...
		for(int i=0;i<N;i++) {
			...
			if(val1<125)label2+="4";
			else if(val1>275)label2+="1";
			else label2+="2";

			label2+="-Date=";
			if(val2<200)label2+="12";
			else label2+="24";
	...
	public static void coord2() {
		...
		for(int i=36;i<N;i++) {
			...
			if((i%5) ==0)IJ.saveAsTiff(img2, base3);
		}		
		IJ.saveAsTiff(img2, base3);
	}
	...
	public static void coord1() {
		...
		for(int i=266;i<N;i++) {
			...
			img2.getStack().setSliceLabel(label, i+1);
			if((i%5) ==0)IJ.saveAsTiff(img2, base2);
		}		
