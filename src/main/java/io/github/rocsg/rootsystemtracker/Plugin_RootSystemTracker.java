...
public class Plugin_RootSystemTracker extends PlugInFrame{
	...
	public static String versionFlag="Handsome honeysuckle v1.5.1  2023-04-18 19:42 Split-root-03-typeExp-Feature";
	...
	public void run(String arg) {
		...
		if(arg.equals("Dev ML1")) {
			...
			if(VitiDialogs.getYesNoUI("Go on a previous experiment ?", "Use an ongoing series ? ")) {
				String csvPath=VitiDialogs.chooseDirectoryNiceUI("Select the processing main directory containing InfoSerieRootSystemTracker.csv file", "OK").replace("\\","/");
				csvPath=new File(csvPath, "InfoSerieRootSystemTracker.csv").getAbsolutePath();
				if(!new File(csvPath).exists()){IJ.showMessage("No csv file there");return;}
				...
				if(VitiDialogs.getYesNoUI("Start from an existing inventory ?", "Start from an existing inventory ?")) {
					inventoryPath=VitiDialogs.chooseOneImageUIPath("Choose A_main_inventory.csv", "A_main_inventory.csv");
