...
public class RstBox {
	...
	//Demo function for getting the length of the primary roots of a box with respect to the successive time points
	public void demo1() {			

		rootModel.writeRSML3D(new File("/home/rfernandez/Bureau/rsmlTest.rsml").getAbsolutePath().replace("\\","/"), "",true,false);
		//Display some information about the Root System
		for(Root r: rootModel.rootList){
			System.out.println(r.order+" "+r.plantNumber+(r.getParent()!=null ? r.getParent().plantNumber :""));
		}
		...
		//Compute some traits about it, and get it as an array [trait from 0 to 2][timepoint from 0 to Nt-1]
...
*/
		...
		if(usecase==1) {
			...
			currentBox=new RstBox("/home/rfernandez/Bureau/A_Test/RootSystemTracker/TestSplit/Processing_of_TEST1230403-SR-split/230403SR055");
		}
		if(usecase==2) {
		...
		imgView.show();
		//Uncomment this if you want to wait for 5 seconds, then close the view automatically
		...
		//In case the original rsml was messy for investigations (root order or things like this, write a new version (it will be ordered, prim from left to right, and lat from top to bottom)
		currentRootModel.writeRSML3D(new File("/home/rfernandez/Bureau/rsmlTest.rsml").getAbsolutePath().replace("\\","/"), "",true,false);
		...
		//Display it in the console
		for(int i=0;i<tab.length;i++)System.out.println("Plant "+i+" has "+tab[i]+" lateral roots");

		
		
		//Get the leftmost plant primary root
		Root primOfPlant0=currentRootModel.getPrimaryRootOfPlant(0);//Can be used from plant 0 to plant 4		
		//Compute some traits about it, and get it as an array [depth / length][timepoint from 0 to Nt]
		...
		//Compute some traits about it, and get it as an array [depth / length][timepoint from 0 to Nt]
		double[][]traitsOfFirstLatOfPlant4= currentRootModel.getTipDepthAndRootLengthOverTimesteps(lateralsOfPlant4[0]);
		...
		//Save the traits of the primary as a CSV file
		VitimageUtils.writeDoubleTabInCsv(traitsOfFirstLatOfPlant4, "/home/rfernandez/Bureau/traitsTestFirstLatOfPlant4.csv");
	...
	public RstBox(String path) {
		...
		if(path==null || path.length()==0)path=VitiDialogs.chooseDirectoryNiceUI("Choose dir box", "OK");
		...
		//Verify if the processing went to the step 65, i.e. there is a file named 65_times.tif
		...
		if(new File(path,"61_graph_expertized.rsml").exists()) {
			rootModel=RootModel.RootModelWildReadFromRsml(new File(path,"61_graph_expertized.rsml").getAbsolutePath().replace("\\", "/"));
		}
		else if(new File(path,"61_graph.rsml").exists()) {
			rootModel=RootModel.RootModelWildReadFromRsml(new File(path,"61_graph.rsml").getAbsolutePath().replace("\\", "/"));
		}
		else{
	...
	//Method for viewing the superposition of the image and the model, in the way that io.github.rocsg.fijiyama.rsml.RsmlExpert.java does
	...
	public ImagePlus projectRsmlOnImage(RootModel rm) {
		...
		for(int i=0;i<Nt;i++) {
			ImagePlus imgRSML=rootModel.createGrayScaleImageWithTime(imgReg,1,false,(i+1),true,new boolean[] {true,true,true,false,true},new double[] {2,2});
