...
//					int[]coords=ccLast.getNextSourceFromFacetConnexion(ccLast.bestIncomingActivatedEdge()); //currentSource;
	...
	public static void establishDataFromStartForEachNodeAndExcludeBasedOnStatistics(SimpleDirectedWeightedGraph<CC,ConnectionEdge> graph,ImagePlus sampleImgForDims,PipelineParamHandler pph,int indexBox) {
		...
		for (CC cc:graph.vertexSet()) {
			...
			if(!cc.trunk && cc.bestIncomingActivatedCC()==null) {
				...
				if(ct.euclidianDistanceToCC(cc)>pph.getMeanSpeedLateral()*10 && (cc.bestIncomingEdge()==null || (succ>9) ) ) { // TODO : The second condition is really questionable. There should be specific tests here to improve performances
	...
	//						System.out.println("B REJECT Concluding too small");
						...
						for(CC cc : ccToTest) {
							...
							if(ccLast.euclidianDistanceToCC(cc)>2*pph.getMaxSpeedLateral())continue;
...
//		for(CC cc:tabDel)cc.lightOffLateralRoot();
	...
	public static void reconnectDisconnectedBranches_v2(ImagePlus img2, SimpleDirectedWeightedGraph<CC,ConnectionEdge> graph,PipelineParamHandler pph,int formalism,boolean workAlsoBranches,boolean hack) {
		...
		while(!finished) {
			...
			for(int i=0;i<Nstop;i++) {
	            for(int j=0;j<Nstart;j++) {    
	            	boolean debug=(false && (i==47) && (j==205));
	...
	public static double weightingOfPossibleHiddenEdge_v2(ImagePlus img,SimpleDirectedWeightedGraph<CC,ConnectionEdge> graph,CC ccStop,CC ccStart,PipelineParamHandler pph,boolean debug) {
		...
		//Speed score
		double dtCross=ccStart.day-ccStop.day;//TODO : This should be delta hours
		if(dtCross==0)dtCross=0.7;//TODO : This should be math.min (delta sup, delta inf)
		...
		if(nStop>1) {
			...
			if(dtStop==0)dtStop=0.7;//TODO : This should be math.min (delta sup, delta inf)
		...
		if(nStart>1) {
			...
			if(dtStart==0)dtStart=0.7;//TODO : This should be math.min (delta sup, delta inf)
		...
		//Connection score
		int nbSteps=areConnectedByPathOfCC_v2(graph,ccStop,ccStart,pph,debug);//TODO : check, and adjust to make it double
		int lNorm=3;
		tabGamma[5]=(1.0/lNorm)*Math.abs(nbSteps-dtCross-2.5)+(dtCross<2 ? 0 : 0.15*(dtCross-1));//TODO : corresponding to upper
		...
		tabGamma[8]=costDistanceOfPathToObject(img,ccStop.x ,ccStop.y,ccStart.x,ccStart.y);//TODO : check ?
		tabGamma[9]=0.5;
		double maxSpeedLat=pph.getMaxSpeedLateral();//TODO : to multiply by actual delta
		if(  ((ccStop.euclidianDistanceToCC(ccStart))/(Math.max(0.7, ccStart.day-ccStop.day))) >maxSpeedLat) {//TODO : check
			tabGamma[9]=5*(  ((ccStop.euclidianDistanceToCC(ccStart))/(Math.max(0.7, ccStart.day-ccStop.day)))-maxSpeedLat)/maxSpeedLat;//TODO : check
		}
...
//10 si nPixels=1 , 7 si ça vaut 2 , 5 si ça vaut 3, 4 si ça vaut 4, 3 si ça vaut 5, 		
	...
	///////////////////////////
	//BASICALLY UNSAFE ZONE////
	///////////////////////////	
	public static double getCostOfAlignmentOfLastPointGivenTwoFirst(CC cc1,CC cc2,CC cc3,boolean debug) {
		int delta12Day=Math.abs(cc1.day-cc2.day);//TODO : using real time ?
		int delta23Day=Math.abs(cc3.day-cc2.day);//TODO : using real time ?
		...
		while(!finished) {
			...
			for(int i=0;i<visited.size();i++) {				
				...
				for(ConnectionEdge edge : graph.outgoingEdgesOf(ccTemp)) {
					...
					if(!ccTrial.trunk && ccTemp.trunk) {
						double scoreAdd=ccTrial.euclidianDistanceToCC(ccTemp.lastCCinLat)/pph.getMeanSpeedLateral();//TODO : make it depends to actual delay
...
//		imgDatesTmp.show();
		...
		if(doImages) {
			...
			ImagePlus graphs;
			if(doDebugImages) {
				ImagePlus[]graphsImgs=new ImagePlus[3];
				ImagePlus[]backImgs=new ImagePlus[3];
